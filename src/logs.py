...
class Logs(Updatable):
    ...
    def __init__(self, top_left: Point, size: Vector) -> None:
        ...
        self.logs_text = Text("Logs:", top_left[0], top_left[1], color.BLACK, self.TEXT_DY - 2)
    ...
    def draw(self) -> None:
        ...
        for i, text in enumerate(reversed(self.texts)):
            text.draw(self.cur_time, self.top_left[1] - i * self.LINE_SIZE - self.TEXT_DY)
    
    def log(self, txt: str) -> None:
        self.texts.append(LogLine(txt, Text(txt, self.top_left[0], 10, color.BLACK, self.LINE_SIZE - 4), self.cur_time))
        self.pop_old()